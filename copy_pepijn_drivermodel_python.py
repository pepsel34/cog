...
def runSimulations(nrSims=25):
    conditions = ["word", "sentence", "drivingOnly", "none"]
    totalTime = []
    meanDeviation = []
    maxDeviation = []
    Condition = []
...
def plotSimulations(totalTime, meanDeviation, maxDeviation, Condition):
    conditions = ["word", "sentence", "drivingOnly", "none"]
    symbols = {"word":"o","sentence":"^","drivingOnly":"s","none":"x"}
    colors = {"word":"blue","sentence":"red","drivingOnly":"green","none":"black"}
    ...
    for cond in conditions:
        ...
        # raw trial points
        plt.scatter([totalTime[i] for i in idx], [maxDeviation[i] for i in idx], 
                    c='grey', marker=symbols[cond], alpha=0.5, label=f"{cond} trials" if cond=="word" else "")
        ...
        plt.errorbar(meanTime, meanMax, xerr=stdTime, yerr=stdMax, fmt=symbols[cond], 
                     c=colors[cond], markersize=10, capsize=5, label=f"{cond} mean ± std")

    plt.xlabel("Total trial time (ms)")
    plt.ylabel("Max lateral deviation (m)")
    plt.title("Simulation results (100 trials per condition)")
    plt.grid(True, linestyle="--", alpha=0.6)
       # --- Legend using proxy artists --- #
    legend_elements = []
    for cond in conditions:
        # Raw trials
        legend_elements.append(mlines.Line2D([], [], color='grey', marker=symbols[cond], linestyle='None', label=f'{cond} trials'))
        # Means
        legend_elements.append(mlines.Line2D([], [], color=colors[cond], marker=symbols[cond], linestyle='None', markersize=10, label=f'{cond} mean ± std'))
